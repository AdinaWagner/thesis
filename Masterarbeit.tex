...
The frontal eye fields (FEF) are a core part of the endogenous, dorsal attention system, and a site of convergence for motor tasks in eye movements, and higher cognitive functions in visuospatial attention. Their differential role in attentional modes and their right-hemispheric dominance have only been studied in highly controlled experimental settings, and lack a generalization to the competing attentional demands of naturalistic stimulation. Therefore, this thesis studies the functional specificity of the left and right FEF with regard to attentional mode during stimulation with the audiovisual Hollywood movie 'Forrest Gump'.
For this aim, a multi-step procedure was employed: The frontal eye fields are localized by utilizing unconstrained gaze during movie watching in a general linear model contrasting horizontal and vertical eye movements. For a measure of visuospatial attentional control, a multidimensional, geometric algorithm for scanpath comparison is used in conjunction with fixation data. Simultaneously, a novel method to derive functional specificities of regions of interests is implemented and validated on data from a standard localization paradigm and a movie watching paradigm of the studyforrest dataset. This method is henceforth used to model the functional specificities of the frontal eye fields with two attentional regressors from the multimatch algorithm. \newline
The results provide a general proof-of-principle for the ROI specificity method, and show that FEF localization from unconstrained eye movements is generally possible. The visuospatial attention measure bears practical and theoretical constraints that limit the informative value of the ROI specificity analysis of the FEF.
\newline
...
\begin{figure}[H]
	...
	\caption[Saccade characteristics during movie watching]{Direction and lengths characteristics of all extracted saccades.}
...
As evident from table \ref{tab:masks}, the location of the derived FEF masks varies between subjects. In general, however, the obtained coordinates lie in Brodmans Area 6, which corresponds to human fMRI studies (\cite{vernet2014corrigendum}), and the average activation maxima correspond to probable FEF locations found by others as well (e.g. \cite{paus1996location}; \cite{tehovnik2000eye}; \cite{connolly2002human}, Luna et al. (1998), cited by \textcite{vernet2014corrigendum}). The large range in individual x-coordinates suggests that activation in both the ventrolateral region, generating shorter saccades (2-15$^\circ$, \cite{bruce1985primate}), and mediodorsal region, generating longer saccades (13-25$^\circ$, \cite{bruce1985primate}), has been captured by the contrast. This is in accordance to figures \ref{fig:saccs} and \ref{fig:sacc_count}, which show that saccades of up to 30$^\circ$ we evoked by the movie stimulus. \\
...
\begin{quotation}
	{\footnotesize "Being involuntary, exogenous control
...
\section{Methods}
The first part of this section is dedicated to the MultiMatch algorithm. It gives a comprehensive overview of how the algorithm works, and describes how it was used for the studyforrest data. The second part elaborates on the application of the ROI specificity analysis for the study of the frontal eye fields.
...
The following section gives a brief overview of the general MultiMatch method. An overview in pseudocode is outlined in algorithm \ref{algo:multimatch} in the appendix. \newline
...
\begin{figure}
	...
	\caption[Correlation structure of multimatch measures]{{\small Correlation structure of the resulting multimatch measures.}}
	\label{fig:simcor}
\end{figure}
