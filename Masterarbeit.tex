...
As table \ref{tab:masks} shows, however, this approach was unable to identify masks for 20 out of the 30 masks. For remainder of FEFs, a grouplevel mask obtained from the result of the dataset-level was substituted.

\section{Results}

Table \ref{tab:masks} summarizes minimum and maximum coordinates as well as the location with the highest activation of obtained masks per participant and hemisphere. Figure \ref{fig:FEF}
...
include stuff about
\begin{itemize}
	\item{occurance of larger and small saccades, and the proportion of directions}
	\item{The thresholds and steps used for mask generation}
	\item{a table describing found clusters}
	\item{a figure showing the results}
\end{itemize}
...
include:
\begin{itemize}
	\item{discuss troubles with localization}
...
This analysis uses the custom preprocessed BOLD files from the movie experiment (see section \ref{section:generalmethods}), the eye tracking classification by REMoDNaV, the FEF ROI masks derived in the previous chapter, the annotation confounds by \textcite{hausler2016annotation}, and the stimulus confounds files already used in chapter \ref{c2}.
...
\subsection{Investigating functional specificity differences in left and right FEF}
\textbf{TODO: we need some text that described what we did here.}
 
The correlation structure within the multimatch measures proofed to be suboptimal (further explored in section \ref{sec:res_mm}). As evident in figure \ref{fig:dist_hist}(c), only a maximum of two dimensions 
simultaneously are mutually uncorrelated. Due to this constraint, two dimensions with a straightforward interpretation were subselected for the analysis, duration and position. The position dimension indicates the similarity with regard to points of interest in the movie: Do participants fixate the same parts of the screen? The duration dimensions on the other hand may be more indicative of employed exploration strategies: Do participants fixate points of interest similarly long?
